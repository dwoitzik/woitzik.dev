...
This article walks through how to break the Pending deadlock programmatically, strip out static API keys with Identity Chaining, and wire up Private DNS so the whole stack survives a Zero-Trust audit.

**[Get the AzAPI Auto-Approve workaround free on GitHub 🐙](https://github.com/dwoitzik/terraform-azurerm-openai-rag)**
...
`Cognitive Services OpenAI User` is the least-privileged role for this use case - it allows AI Search to submit inference requests but cannot modify the OpenAI account configuration. Least-privilege RBAC is a hard requirement under ISO 27001 Annex A.8 and [NIS2 Article 21](/blog/nis2-article-21-azure-terraform/).
...
`registration_enabled = false` is intentional - automatic DNS registration conflicts with centralized Private DNS Zone management in [Hub & Spoke](/blog/azure-terraform-hub-spoke-zero-trust/) environments. If you are running this inside an existing Hub & Spoke topology, the DNS zones should live in the Hub and be linked cross-subscription, not deployed per-workload.
...
The AzAPI trick above is worth knowing if you're stuck with a Shared Private Link approval loop - it's a real, general-purpose pattern. But the shipped module went a step further and dropped the workaround entirely: instead of a Shared Private Link needing programmatic approval, it puts both services behind proper Private Endpoints and has AI Search authenticate to OpenAI via Managed Identity + RBAC. No approval step to automate because there's nothing pending to approve. Full VNet injection, Private DNS, and Identity Chaining, all in a single free, MIT-licensed module.

If the DNS/private-networking side of this is the unfamiliar part, [Netzwerke: Verstehen, Einrichten, Administrieren\*](https://amzn.to/4f3kTar) covers the underlying concepts well.
