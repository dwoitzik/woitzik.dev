...
description: "My Proxmox host froze under load three times in one week. The root cause wasn't software — it was a marginal PSU triggering TDP throttling, which exposed a ZFS ARC deadlock. Here's the full chain from symptom to fix."
date: "2026-07-30"
...
The first time, I blamed a kernel bug. The second time, I blamed ZFS. The third time, I found the actual chain: a marginal PSU causing CPU throttling under load, which triggered a ZFS ARC deadlock that locked the entire host.
...
My [BMAX Mini PC\*](/go/4bv3yF1) — an older B5A Pro (Ryzen 7 5825U) — ships with a 90W PSU. (The linked current model has moved on to the newer 8745HS chip; the PSU/TDP numbers below are specific to my older unit.) Under normal operation, the Ryzen 7 5825U draws 15W TDP and the NVMe pulls another 8W. But when all cores are loaded — k3s workers, Ollama inference, OCR workers — the CPU can boost well above base TDP.

I added power monitoring via a [TP-Link Tapo P110\*](/go/3STKPwD) to track actual draw:
...
In ZFS 2.4.1, the version running on this host, the ARC eviction path can block on locks held by a stalled txg commit — a deadlock class OpenZFS has documented before under memory pressure ([txg_sync hung task when shrinking ARC, openzfs/zfs#4319](https://github.com/openzfs/zfs/issues/4319), [ARC eviction thread blocking on allocations, openzfs/zfs#12985](https://github.com/openzfs/zfs/pull/12985)). This creates a circular wait: ARC eviction needs txg to complete, txg needs I/O bandwidth, I/O is delayed by CPU throttling, CPU throttling is caused by the PSU sag, and the PSU sags because everything is running at once.
