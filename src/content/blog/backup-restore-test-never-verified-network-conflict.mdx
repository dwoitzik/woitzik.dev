...
The container chosen was `ct-srv-atlantis-01` - not because it was low-stakes exactly, but because its data (Terraform plan history, lock state) is reconstructible if something went wrong, unlike, say, Nextcloud's actual file storage. Backups go to a local [Seagate 2TB External HDD\*](https://amzn.to/4vxwLpY): deliberately local, because depending on the thing you're backing up *away from* failing defeats the purpose.
...
While writing up this test, a second problem surfaced - not from testing, but from cross-checking Tier 1's other listed recovery path against a separate finding from the same week: the Google Drive offsite sync had its own throttling and drift issues (REL-051), diagnosed the same week and covered in a separate write-up. The runbook's fallback for "total PBS storage loss" pointed at that same offsite sync as the recovery source.
