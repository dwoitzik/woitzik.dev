...
This is the Terraform Circular Dependency. This article explains exactly why it happens, how to break the loop cleanly, and why solving the cycle error is only the first half of the battle - before Azure's silent PaaS Trap destroys your workloads.

**[Get the basic Cycle-Error fix free on GitHub 🐙](https://github.com/dwoitzik/terraform-azurerm-firewall-forced-tunneling)**
...
The module binds the Route Table to its own self-contained spoke subnet by default. If you're peering in additional external Spokes rather than using the built-in one, don't duplicate the association resource for each - bind dynamically with `for_each` over a variable map instead:
...
When a new Spoke CIDR is added to `var.spoke_subnet_cidr`, every firewall rule that references `azurerm_ip_group.spokes.id` automatically inherits it. No rule editing required.
...
The repository at the top covers everything in this article - cycle error fix, KMS and AD bypasses, the baseline Route Table structure, dynamic IP Groups, and FQDN baseline policies - free and MIT licensed.

If the routing/NAT concepts here are the unfamiliar part rather than just the Terraform syntax, [Netzwerke: Verstehen, Einrichten, Administrieren\*](https://amzn.to/4f3kTar) is the German-language reference I keep going back to.
