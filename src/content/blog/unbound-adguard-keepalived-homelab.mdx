...
This is the DNS architecture running on two Raspberry Pi 4B edge nodes ([starter set\*](https://amzn.to/4wNS0oI)) in my homelab. If you're building this today, go straight for the [Raspberry Pi 5\*](https://amzn.to/4wETNfv), the performance jump for containerized workloads is massive: Unbound as a recursive resolver, AdGuard Home for filtering, and Keepalived for automatic failover. The whole stack is managed with Ansible.
...
VRRP uses multicast. If your switch filters multicast between ports ([MikroTik\*](https://amzn.to/4w08b21) does by default), you need to permit `224.0.0.18` on the VLAN carrying the Pi nodes.
...
DNS control is the foundation of any Zero-Trust network - on-premises or in the cloud. The same internal DNS infrastructure that serves client queries also drives TLS certificate automation — [cert-manager on k3s uses DNS validation](/blog/cert-manager-wildcard-k3s-cloudflare/) for wildcard Let's Encrypt certificates, making DNS reliability a prerequisite for both connectivity and encryption. In Azure, the equivalent of this setup is Azure Private DNS Zones with Private Link resolvers.
