...
This is the DNS architecture running on two Raspberry Pi 4B edge nodes ([starter set\*](/go/4wNS0oI)) in my homelab. If you're building this today, go straight for the [Raspberry Pi 5\*](/go/4wETNfv), the performance jump for containerized workloads is massive: Unbound as a recursive resolver, AdGuard Home for filtering, and Keepalived for automatic failover. The whole stack is managed with Ansible.
...
VRRP uses multicast. If your switch filters multicast between ports ([MikroTik\*](/go/4w08b21) does by default), you need to permit `224.0.0.18` on the VLAN carrying the Pi nodes.
