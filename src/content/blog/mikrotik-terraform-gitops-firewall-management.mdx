...
My <AffiliateLink href="https://amzn.to/4w08b21">MikroTik RB5009</AffiliateLink> has no manual RouterOS configuration. Every firewall rule, every VLAN interface, every DHCP static lease, every NAT port-forward — all of it lives in Terraform, reviewed via Atlantis pull requests, and applied through the same GitOps workflow that manages the Kubernetes cluster. This is the delivery pipeline underneath [the zero-trust default-deny firewall policy](/blog/mikrotik-zero-trust-firewall-terraform/) and [the VLAN matrix](/blog/mikrotik-vlan-filtering-terraform-proxmox/) — this article covers the Atlantis plumbing, not the rules themselves.
