...
What it **deliberately doesn't cover**: bridge-mode containers like Unbound (5335) and node_exporter (9100). Docker DNATs traffic to these *before* it ever reaches the host's INPUT chain - this firewall's table never sees that traffic, confirmed by live testing, not just by reading documentation about how Docker's iptables integration works. Restricting bridge-mode container ports would require rules in Docker's own `DOCKER-USER` chain, with careful IPv4/IPv6 handling to avoid breaking container egress. I deferred this: [MikroTik\*](https://amzn.to/4w08b21) [already segments these Pis from the wider internet at the network layer](/blog/mikrotik-zero-trust-firewall-terraform/), and the mistake-risk of getting `DOCKER-USER` chain rules wrong on a live DNS server outweighed the marginal security benefit of restricting traffic that's already internal-only.
