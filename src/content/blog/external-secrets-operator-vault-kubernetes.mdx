...
The proper solution is an external secret store: a system specifically designed for secret storage, with access control, audit logging, and rotation built in. [HashiCorp Vault](https://www.vaultproject.io/) is the most common choice. [External Secrets Operator](https://external-secrets.io/) bridges Vault to Kubernetes - syncing secrets into the cluster without storing them in Git. The whole setup runs on my [BMAX Mini PC\*](https://amzn.to/4bv3yF1)-based k3s cluster.
