...
Renovate kept getting OOMKilled on the [BMAX Mini PC\*](/go/4bv3yF1) running my entire k3s cluster. The Kubernetes CronJob would start, run for a while, and then die with exit code 137. The natural assumption: not enough memory. Bump the limit. Problem solved.
