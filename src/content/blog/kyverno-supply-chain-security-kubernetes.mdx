...
Kubernetes has no opinion about what you run. You can deploy a container with no resource limits, no security context, root access to the host filesystem, and an image tagged `:latest` that changes every week - and the scheduler will place it without complaint. In a homelab on a [BMAX Mini PC\*](https://amzn.to/4bv3yF1), that's annoying. In a production cluster, it's a compliance failure.
