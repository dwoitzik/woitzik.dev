...
**Update (2026-09-05):** this cluster has since moved Garage's storage off Longhorn onto a
static host-NFS-backed PV/PVC pair — a separate post covers that migration and the reasoning
behind it. The Velero/Garage setup described below (architecture, `replication_factor = 1`,
the Deployment itself) is unaffected either way; only the two PVCs' `storageClassName` changed.
Left as-is below since it was accurate for the Longhorn-based setup this cluster ran at the
time and the backup logic itself didn't change.

Every Kubernetes cluster needs a backup strategy. For a homelab running on bare metal, a [BMAX Mini PC\*](/go/4bv3yF1) with Longhorn for distributed storage, the options are limited: `etcd` snapshots cover cluster state but not persistent volumes, and MinIO is the standard S3 target for Velero - but MinIO is large, opinionated, and overkill for a single-node homelab.
...
With this setup, a complete cluster rebuild from scratch - fresh k3s installation, ArgoCD, and a `velero restore` - takes under 30 minutes. That's the practical test for whether your backup strategy actually works. I [ran that exact test](/blog/backup-restore-test-never-verified-network-conflict/) against a scratch VM and found a network conflict the documentation never mentioned.

Backup-first is the default posture on Azure too: Azure Backup, geo-redundant storage, and immutable blob policies exist because the same failure modes you see in a homelab scale to enterprise data.
