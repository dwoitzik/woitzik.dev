...
CPU inference is slower (qwen2.5:7b takes ~30 seconds per inference vs ~8 seconds on the iGPU), but it doesn't crash. Stability over speed for a homelab document processing pipeline — the same trade-off that governs [what gets to auto-update vs. what needs a human](/blog/operating-model-auto-update-human/) on this cluster: a slower, boring path beats a fast, unstable one.
