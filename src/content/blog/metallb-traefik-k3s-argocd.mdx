...
Cloud Kubernetes clusters get LoadBalancers for free. You create a Service of type `LoadBalancer`, and within seconds your cloud provider hands you a public IP. On a bare-metal K3s cluster running on Proxmox VMs (a [BMAX Mini PC\*](https://amzn.to/4bv3yF1) with 64GB handles the whole cluster), that request hangs in `<pending>` forever. Keep your rack tidy from the start, [Velcro Cable Ties\*](https://amzn.to/4bnvY3K) are worth every cent when you start adding switches and patch panels.
...
**`tlsStore.default.defaultCertificate`** - sets the [wildcard certificate](/blog/cert-manager-wildcard-k3s-cloudflare/) as the default TLS certificate for all Ingress resources that don't specify their own. Every service behind Traefik gets HTTPS automatically.
...
The same principles - centralized ingress, TLS termination at the edge, declarative configuration - apply directly to enterprise Azure environments. In Azure, the equivalent is an Application Gateway or Azure Firewall in front of a private AKS cluster.
