...
date: "2026-05-04"
tags: ["MikroTik", "Terraform", "Networking"]
---
...
If you have ever tried to configure VLANs on a MikroTik router, you know the pain. I run the [RB5009\*](https://amzn.to/4w08b21) specifically for its hardware-offloaded VLAN switching, cheaper models like the hEX do VLAN filtering in software, which introduces latency under sustained trunk traffic. Transitioning from the legacy switch-chip VLAN methods to the modern **Bridge VLAN Filtering** often results in completely locking yourself out of the router.

When you add a virtualization host like Proxmox into the mix-which requires a trunk port passing multiple tagged VLANs-the complexity multiplies. Label your cables early, a [Brother Labelmaker\*](https://amzn.to/4wcr4PG) saved me hours tracing connections during a VLAN migration. Doing this via the WinBox GUI is a recipe for configuration drift and late-night network outages.

In this deep dive, I will show you how to tame MikroTik VLANs using Infrastructure as Code (Terraform). We will build a dynamic, scalable L2 network architecture that includes a Proxmox trunk port, dedicated management access, and hardware-offloaded access ports for edge nodes like the [Raspberry Pis\*](https://amzn.to/4wNS0oI) ([hardening guide](/blog/raspberry-pi-edge-hardening-watchdog-fail2ban-nftables/)).
...
**The Proxmox Trunk:**
Our Proxmox server (`ether5`) needs to receive tagged traffic so the virtual machines can reside in different VLANs. We leave it on the default `pvid = 1` - this port carries VLANs by explicit tag, so the pvid only matters for the rare untagged frame, and nothing on this trunk is meant to communicate over untagged VLAN 1 in the first place.
...
By treating network infrastructure as code, we transform a fragile, click-heavy configuration into a robust, auditable state. Proxmox gets its tagged trunks, hardware edge devices get their native access ports, and the router maintains secure L2 isolation. With the VLAN layer in place, the next step is building [L3 firewall rules on top](/blog/mikrotik-zero-trust-firewall-terraform/) — default-deny forward chains, micro-segmented allow rules, and NAT for public-facing services.

Define the desired state, let the provider compute the diff — that declarative loop is exactly what makes enterprise Azure resource deployments auditable, and it's the same loop running here in your edge router.
