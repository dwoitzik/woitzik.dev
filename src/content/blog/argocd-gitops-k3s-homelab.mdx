...
**Update (2026-09-09):** this specific cluster's persistent storage has since moved off Longhorn
onto NFS (same change documented in the [K3s/Authelia/Longhorn homelab article](/blog/k3s-authelia-proxmox-homelab/)).
The `kubernetes/system/longhorn/` and `longhorn-config/` Applications below no longer exist in the
live repository. The GitOps mechanics this article is actually about - the App-of-Apps pattern,
the Helm-chart-plus-config-Application split, the bootstrap sequence - are unaffected by that
change and still accurate; Longhorn is left in place below purely as the worked example of
"Pattern 1 / Pattern 2," which remains a correct illustration for anyone running it on genuinely
distributed nodes.

Most Kubernetes tutorials end with `kubectl apply -f`. You deploy something, it works, and you move on. Three weeks later you have no idea what's running in your cluster, why it's configured that way, or how to recreate it if something breaks. My cluster runs on a [BMAX Mini PC\*](/go/4bv3yF1): small enough to sit in a 10" rack, powerful enough for a full k3s stack.
...
GitOps doesn't change size when you move to Azure: Terraform still owns the infrastructure layer and ArgoCD or Flux still drives the application layer, only the platform underneath becomes AKS.
