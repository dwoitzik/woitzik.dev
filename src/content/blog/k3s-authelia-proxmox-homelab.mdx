...
**Update (2026-09-09):** this cluster has since moved persistent storage off Longhorn onto NFS.
All three k3s nodes here run as VMs on a single physical host, so Longhorn's replica-distribution
guarantee was never real — every "replica" landed on the same underlying NVMe. When a node dropped
out briefly, Longhorn's RWO failover logic tried to attach the volume on a second node while the
first was still recovering, producing a Multi-Attach error that needed a manual detach to clear —
overhead a single-host cluster doesn't need to carry. NFS doesn't have an attachment state to
conflict, so that failure mode is gone. Mistake 2 below (Longhorn + iSCSI) and the Longhorn
Application manifest are left as-is - the troubleshooting is accurate for anyone running Longhorn
on genuinely distributed nodes, just not what this specific cluster runs anymore.

Most Kubernetes homelab guides stop at "kubectl get pods" and call it a day. This one doesn't.

This article documents a full production-grade homelab stack: three K3s nodes provisioned via Terraform on Proxmox, running on a [BMAX Mini PC\*](/go/4bv3yF1) with 64GB RAM, GitOps-managed with ArgoCD, persistent storage via Longhorn, and Authelia as a proper SSO gateway in front of every service. The kind of setup you'd actually trust to run real workloads.
...
Traefik runs in `kube-system` managed by ArgoCD, with HTTP-to-HTTPS redirect enforced at the ingress level and a [wildcard certificate from cert-manager](/blog/cert-manager-wildcard-k3s-cloudflare/) as the default TLS store:
...
For remote access to this stack, I later added [Headscale as a self-hosted Tailscale control plane](/blog/headscale-oidc-k3s-authelia/) — it uses the same Authelia instance for OIDC authentication, so VPN access and service SSO share one identity layer.
