...
A Kubernetes cluster without observability is a black box. You deploy services, they run - until they don't. When something breaks at 2am, you need metrics, logs, and alerts that actually tell you what happened. The whole stack runs on a [BMAX Mini PC\*](https://amzn.to/4bv3yF1) with a [MikroTik RB5009\*](https://amzn.to/4w08b21) handling the network.
...
Keep metrics, logs, and traces as three separate layers and the tooling becomes interchangeable: Azure Monitor, Log Analytics, and Application Insights map onto the same model this stack uses, so dashboards and alert rules carry over almost unchanged.
