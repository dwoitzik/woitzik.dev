...
This is the implementation running on my bare-metal k3s cluster ([BMAX Mini PC\*](https://amzn.to/4bv3yF1) with 64GB), based directly on the multi-window multi-burn-rate approach from the [Google SRE Workbook](https://sre.google/workbook/alerting-on-slos/). I also track server power draw via a [Tapo P110\*](https://amzn.to/3STKPwD): useful for correlating outages with power events.
...
- A 30x burn rate means the budget is being consumed 30 times faster than the rate that would exactly exhaust it over the full 30-day window
- Time to exhaustion at a sustained 30x rate: 43.8 minutes ÷ 30 × (elapsed detection window factored in) ≈ 50 minutes

50 minutes to act. Page.
...
**The math:** `6 × 0.001 = 0.006` - a 0.6% error rate. At a sustained 6x burn rate, the monthly budget is exhausted in about 4 hours - enough runway that a ticket during the shift is the right response, not a page. The `for: 15m` means it must sustain this rate for 15 minutes before firing, which filters transient dips.
...
Swap Prometheus for Azure Load Balancer health probes or Application Gateway and the SLI stays the same: probe success rate. The recording rules and alert thresholds transfer as-is — only the metric source changes.
