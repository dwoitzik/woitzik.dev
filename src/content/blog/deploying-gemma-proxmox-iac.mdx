...
This guide covers how to automate a complete local AI stack on Proxmox VE using **Terraform** for the infrastructure and **Ansible** for provisioning. We will cover the quirks of the Proxmox Terraform provider, setting up Ollama, and deploying Open-WebUI as our frontend. 
...
*   **CPU:** AMD Ryzen 7 5825U (8C/16T), runs on a [BMAX Mini PC\*](https://amzn.to/4bv3yF1) (the 8745HS successor is the current equivalent)
*   **RAM:** 64 GB DDR4 3200 MT/s ([Crucial SODIMM Kit\*](https://amzn.to/3QZejZn))
...
We use Terraform (via the `bpg/proxmox` provider) to spin up dedicated, unprivileged LXC containers. To keep the environment secure and segmented, the containers are split across different [VLANs](/blog/mikrotik-vlan-filtering-terraform-proxmox/).
...
Terraform for provisioning, Ansible for configuration — the workflow that stands up this edge node is the same one that stands up an enterprise cloud environment. Host an LLM in Azure and you answer the same isolation questions: where does the model endpoint live, what can reach it, and how is [AI Search / OpenAI traffic](/blog/azure-rag-shared-private-link-automation/) kept off the public internet.
