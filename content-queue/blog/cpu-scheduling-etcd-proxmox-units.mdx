...
The fix: `cpu.units = 2048` on k3s VMs, giving them 2x the CPU scheduling priority over every other workload on the host.
...
Without CPU priority, etcd's fdatasync also competes for CPU time with these workloads. The kernel's CFS scheduler doesn't know that etcd's fdatasync is more important than Ollama's matrix multiplication — it just sees two processes requesting CPU time and分配 equally.
...
- k3s control-plane needs guaranteed RAM for API server and scheduler (solved by `memory.dedicated = 12284`)
