...
The 3-2-1 backup rule is simple: 3 copies of your data, on 2 different media types, with 1 copy offsite. My homelab implements all three layers. In theory. In practice, each layer has its own failure mode that I discovered only when I needed it — starting with [the Garage S3 target Velero backs into](/blog/velero-garage-k3s-backup/), which is where layer 1 lives.
...
Waiting on a Cloudflare R2 account with real API credentials. R2 has no egress fees and no API rate limiting for the volume this backup produces (~4 GB/day). Activating it also fixes a second problem beyond throttling: [Garage runs inside the same cluster Velero is protecting](/blog/velero-backup-circularity-garage-r2-offsite/), so an in-cluster-only backup target is circular regardless of how fast it uploads.
